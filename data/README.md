...
- 15003467의 `지리정보시스템 상 거리(m)` 컬럼명은 **실측 검증 결과 정확하다**. 거리는 미터가 맞고(격포코스 지오메트리 실측 14.74km = 컬럼 14,745.88m), 시간은 분이 맞다(분으로 해석 시 평균 1.8km/h 로 등산 속도와 일치). 다만 1,890개 중 42개는 컬럼 거리와 지오메트리가 25% 이상 어긋나며, 양방향으로 틀려 어느 쪽도 정답으로 확정할 수 없다 — `geomDistM` 을 따로 추가하고 `distMismatch` 로 표시해 뒀다.
