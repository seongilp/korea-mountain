 ...
 *
 * peek 은 "지도를 보면서 산 이름과 코스 몇 줄만 훑는" 상태라 지도에 7할 이상을
 * 남긴다. full 을 1.0 이 아니라 0.94 로 두는 건, 시트가 화면을 꽉 채우면 뒤에
 * 지도가 있다는 사실 자체가 안 보여서 사용자가 닫는 법을 잃기 때문이다.
 */
 ...
 * 모바일 전용 드래그 바텀시트.
 *
 * 지도 위에 상세를 띄우되 "어느 산 얘기인지"를 잃지 않게 하는 게 목적이라,
 * 전체 화면 모달이 아니라 높이를 조절할 수 있는 시트로 만든다.
 * 자매앱 seoul-vibe 의 같은 컴포넌트를 옮겨 왔다.
 *
  ...
  */
  ...
  return (
    ...
    >
      ...
      {/*
        스크롤은 시트가 갖는다. 안에 들어가는 건 코스 목록과 고도 프로파일이라
        자체 스크롤 컨테이너가 없다. 대신 자식에 h-full 을 주지 않아야 이중
        스크롤이 안 난다.
